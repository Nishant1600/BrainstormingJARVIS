...
#
# Bounds are computed as UTC-aware stamps, not naive local ones. A naive
# `datetime(2, 1, 1).timestamp()` goes through the C `mktime` path, which on
# Windows cannot represent pre-1970 (or far-future) dates and raises OSError
# at import time — breaking every importer on win32. The aware-UTC form is
# pure calendar arithmetic, works on both platforms, and is the more correct
# comparison anyway: `_epoch` parses ISO-8601 UTC stamps into aware datetimes,
# so the gate is now in the same frame as the value it gates (within hours,
# against two years of slack).
