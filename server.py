...
import notifier
import tts
from brain import Brain, BrainConfig, OpencodeBrain, MAX_BOOT_PROJECTS
from speech import Priority, SpeechScheduler

# `actions`, `dialog`, `notifier` and `screen` are the Windows
# implementations outright now (the macOS AppleScript/screencapture halves
# were removed, not branched): no runtime binding is needed, and every
# `server.actions` / `server.dialog` patch point the tests ever used keeps
# working because the names never changed. (browser.py was already
# platform-neutral and stays one.)
...
def _tool_url() -> str:
    """The loopback URL the MCP child dials for `/internal/tool`.

    Built from the server's ACTUAL bind scheme/host/port — recorded into
    the environment by main() right before uvicorn.run — not assumed
    defaults, because the server may be on a non-default port, bound to
    ::1, or serving HTTPS via a self-signed cert (CLAUDE.md's own
    quick-start setup). One function for both config writers (mcp.json
    and opencode.json) so they can never disagree.
    """
    ...
    return f"{scheme}://{connect_host}:{port}/internal/tool"
...
def _write_mcp_config(home: Path) -> Path:
    ...
    gets the endpoint and the token path explicitly here. The URL comes
    from `_tool_url()`, not assumed defaults (see above).
    """
...
def _opencode_server_entry(entry: dict) -> dict | None:
    """One validated user entry in opencode's MCP shape, or None when the
    entry has nothing opencode can start.

    Local entries (`command` plus optional `args`/`environment`) become
    `{"type": "local", ...}` with the command as ONE argv array — never
    split, so a path with spaces survives. Remote entries (`url`, plus an
    optional `headers` mapping) become `{"type": "remote", ...}`. Anything
    else was already refused upstream by `declared_connections`, which only
    lets through entries with a `command` or a `url`; this returns None
    rather than raising for the gap between the two.
    """
    ...
    if isinstance(command, str) and command:
        ...
        server: dict = {"type": "local", "command": argv}
        environment = entry.get("environment")
        if isinstance(environment, dict):
            server["environment"] = {str(k): str(v)
...
def _write_opencode_config(home: Path) -> Path:
    """Generate the brain home's opencode.json: JARVIS's own tools, plus
    whatever the user declared in `<data>/jarvis/connections.json`,
    translated into opencode's shape (verified live against
    `opencode mcp list`).

    The same validation as `_write_mcp_config` runs first — reserved
    names, name shapes, command-or-url presence — because the guarantees
    do not depend on which CLI reads the file. The jarvis entry is
    written LAST for the same reason it is in mcp.json: it cannot be
    displaced.

    Two deliberate differences from the claude file. First, there is no
    `--strict-mcp-config` on this backend: the brain ALSO sees the user's
    global opencode.json servers. That is safe by the standing rule, not
    by isolation — results from non-jarvis servers are untrusted content
    (see brain.untrusted_tool_source), same as web pages. Second, the
    brain's env passes provider keys through (see opencode_env), so only
    the tool URL and token path need declaring here.
    """
    global LAST_CONNECTIONS
    LAST_CONNECTIONS = declared_connections()
    for problem in LAST_CONNECTIONS.problems:
    ...
    servers: dict = {}
    for name, entry in LAST_CONNECTIONS.servers.items():
        translated = (_opencode_server_entry(entry)
                      if isinstance(entry, dict) else None)
        if translated is None:
            log.warning("connections: %s has nothing opencode can start; "
                        "left out", name)
            continue
    ...
    servers[RESERVED_SERVER_NAME] = {
        "type": "local",
        "command": [sys.executable,
                    str(Path(__file__).parent / "jarvis_mcp.py")],
        "environment": {
            "JARVIS_TOOL_URL": _tool_url(),
            "JARVIS_TOOL_TOKEN_FILE": str(data_paths.tool_token_path()),
        },
    ...
    # Same 0600 discipline as mcp.json: the token path and the user's own
    # environment blocks (their Notion token, their GitHub token) live here.
    path.write_text(json.dumps(config, indent=2), encoding="utf-8")
    try:
        path.chmod(0o600)
    except OSError as e:                             # pragma: no cover
        log.warning("could not tighten opencode.json's permissions: %s", e)
    return path
...
async def start_brain_and_speech() -> None:
    ...
    if _JARVIS_RUNTIME == "opencode":
        # The opencode backend reads its MCP servers from opencode.json in
        # the brain home (project discovery picks it up via --dir); there
        # is no --mcp-config flag, so config.mcp_config stays empty.
    ...
    # The opencode backend needs no --mcp-config or --tools on argv (it reads
    # its MCP servers from opencode.json, and the persona from the brain
    # home's CLAUDE.md) — but it shares the config shape, so only the class
    # differs here.
    if _JARVIS_RUNTIME == "opencode":
        brain_instance = OpencodeBrain(config)
    else:
...
async def _announce_needs_you(event: dict) -> None:
    ...
    # human as process data and never formats it into a command —
    # `test_untrusted_text_travels_in_env_never_argv` is the guarantee — so
    # pre-scrubbing it there would hide the real name from the user without
    # protecting anything. The scrubbing above is for the line JARVIS SAYS,
    # which lands in his own context.
...
async def start_session_watcher() -> None:
    ...
    if _JARVIS_RUNTIME == "opencode":
        session_watcher = opencode_watch.OpencodeWatcher(
...
# Shared state
# Which agent runtime spawned runs use. "claude" until the cutover (Phase 6);
# "opencode" opts into `opencode run --format json` children today. Anything
# else falls back to "claude" loudly rather than crashing the boot — a
# misspelled value must not take the server down.
_JARVIS_RUNTIME = os.getenv("JARVIS_RUNTIME", "claude").strip().lower()
if _JARVIS_RUNTIME not in ("claude", "opencode"):
    log.warning("unknown JARVIS_RUNTIME %r; using 'claude'",
                os.getenv("JARVIS_RUNTIME"))
    _JARVIS_RUNTIME = "claude"
run_executor_instance = RunExecutor(
    run_store, max_concurrent=3, runtime=_JARVIS_RUNTIME,
    opencode_path=os.getenv("JARVIS_OPENCODE_PATH") or None)
...
def _jarvis_run_session_ids() -> frozenset:
    ...
    if now - stamped < _RUN_IDS_TTL_SEC:
        return ids
    try:
        run_ids = frozenset(run_store.all_run_ids())
        ...
        run_ids = frozenset()
    if _JARVIS_RUNTIME == "opencode":
        # Run ids and session ids are different namespaces here (the claude
        # backend passes the run id as --session-id, making them one), so
        # each run translates through its first stored event, which carries
        # the opencode session id. Translated once and memoized: session ids
        # never change, so only runs never seen before cost a query.
        ids = frozenset(
            sid for sid in (_opencode_run_session(rid) for rid in run_ids)
            if sid)
    else:
        ids = run_ids
...
_opencode_run_sids: dict[str, str] = {}


def _opencode_run_session(run_id: str) -> str | None:
    """The opencode `ses_*` id one run ran as, or None.

    Read off the run's FIRST stored event — every `--format json` line
    carries it — and memoized for the life of the process. Never raises:
    a run with no events yet (still queued) simply has no session to
    exclude, and an unreadable store must not take the snapshot down.
    """
    if run_id in _opencode_run_sids:
        return _opencode_run_sids[run_id]
    try:
        events = run_store.get_events(run_id, limit=1)
    except Exception:
        return None
    if not events:
        return None
    parsed = opencode_parser.parse_line(events[0].get("payload") or "")
    sid = (parsed or {}).get("sessionID")
    if isinstance(sid, str) and sid:
        _opencode_run_sids[run_id] = sid
        return sid
    return None
    ...
    try:
        ...
        if _JARVIS_RUNTIME == "opencode":
            outcome = await asyncio.to_thread(
                opencode_steer.post_to_session, item.session_id, item.prompt)
        else:
            outcome = await asyncio.to_thread(
                session_steer.post_to_session, item.socket_path, item.prompt)
        record(outcome)
        if outcome == session_steer.SENT:
            # SENT means the bytes left — over the socket, or in a spawned
            # continue-turn — nothing more: no reply is ever read back, so
            # this must not claim the target accepted or even received them
            # — see session_steer.py's note at the auth line on why that
            # can't be known, and opencode_steer.py's module note for the
            # same caveat on this backend.
            #
            ...
            # breath rather than left for the user to discover. (The
            # approval dance is a claude-backend concept; an opencode steer
            # lands as a turn with no such gate, so there is no caveat.)
            if _JARVIS_RUNTIME == "opencode":
            ...
            if _JARVIS_RUNTIME == "opencode":
                await speech.say(
                    f"I couldn't find {_said_name(item)}, sir — it may "
                    f"have finished.", Priority.NORMAL)
            else:
                await speech.say(
...
async def _perform_dialog(item: _StagedDialog) -> None:
    ...
    try:
        ...
        if outcome == dialog.SENT:
            ...
            await speech.say(
                "Windows won't let me send keystrokes there, sir — that "
                "console runs at a higher integrity level than I do.",
                Priority.NORMAL)
...
async def tool_answer_dialog(args: dict) -> str:
    ...
    must have exactly one console this machine can press into, and the key
    must be inside `dialog`'s closed vocabulary. Whether the console is
    actually attached at press time is decided in the staged phase, at the
    moment of pressing — never here.
    """
    ...
    if key is None:
        # Refused before anything is staged, and long before any command
...
async def tool_steer_session(args: dict) -> str:
    ...
    if not _inbound_accepted() and _JARVIS_RUNTIME != "opencode":
        # The brain must not say "sent" when the message will sit unapproved
        # in the other window — which is exactly what happened live.
        # Claude-backend only: an opencode steer lands as a turn with no
        # approval-on-receipt gate, so there is nothing to caveat.
...
#
# Two anchored forms: POSIX `/...` and Windows `C:\...` / `C:/...`. The wall
# is about characters, not platforms — the class below already admits the
# backslash a Windows path needs, so only the anchor was POSIX-only, and
# every `C:\...` project on Windows (including every tmp_path in the test
# suite) was unspeakable: unresolvable, unopenable, unstartable. UNC
# (`\\host\...`) stays refused — rarer than a drive path, and a refusal here
# only ever asks a question, never guesses.
...
def _run_outcome(run: dict) -> str:
    ...
    Either runtime's vocabulary is judged by its own parser: claude
    `assistant`/`result` events by stream_parser, opencode
    `text`/`step_finish`/`tool_use` events by opencode_parser. Both parsers
    answer in the same three strings, so every comparison downstream
    (`== stream_parser.STALLED`, `!= stream_parser.OK`) holds for both.
    A stream matching neither — or a mixture — is OK: no evidence either
    way is never a downgrade.
    """
    run_id = run.get("id")
    if not run_id:
    ...
    try:
        ...
        opencode_events: list[dict] = []
        after = 0
        while len(claude_events) + len(opencode_events) < total:
            ...
            for row in page:
                ...
                if parsed is None:
                    continue
                if parsed.get("type") in ("assistant", "result", "system"):
                    claude_events.append(parsed)
                elif parsed.get("type") in ("text", "step_start",
                                            "step_finish", "tool_use",
                                            "error"):
                    opencode_events.append(parsed)
        if opencode_events and not claude_events:
            return opencode_parser.assess_outcome(
                opencode_events, run.get("result_text") or "")
        if claude_events and not opencode_events:
            return stream_parser.assess_outcome(claude_events,
...
#
# `actions.open_browser` drives exactly two named browsers, so
...
async def tool_enable_session_inbox(args: dict) -> str:
    ...
    Claude-backend only: opencode sessions take a steered message as a turn
    with no approval gate, so there is nothing to enable there.
    """
    if _JARVIS_RUNTIME == "opencode":
        return ("That setting belongs to Claude Code sessions, sir — your "
                "sessions take my messages as turns already, with nothing "
                "to switch on.")
    if _inbound_accepted():
...
def _said_path(root: Path, resolved: Path) -> str:
    ...
    Separators are normalized to `/` before the wall: on Windows the
    relative path spells `src\\billing.ts`, and the wall (rightly) admits
    no backslash — but `/` was already admitted, the two spell the same
    file on Windows, and the spoken label is the more readable of them.
    The filesystem keeps its own spelling; only the sentence is
    normalized, and the wall still judges everything it always judged.
    """
    return _plain_name(_repo_relative(root, resolved).replace("\\", "/"),
                       "that file")
...
async def tool_read_file(args: dict) -> str:
    ...
    # Spoken with forward slashes (see `_said_path`): the wall admits no
    # backslash, and on Windows every relative path has them.
    shown = relative.replace("\\", "/")
    label = _plain_name(shown, "That file")
    if not window.total:
    ...
    body = window.text if label == shown else f"{relative}\n\n{window.text}"
...
# different amounts. `what_is_on_screen` is one window enumeration: which
# app is in front and what its windows are called, a few kilobytes, no
# pixels at all, and it answers "what am I looking at" outright. `look_at_screen` is a
# real picture the brain SEES, about 1,200 tokens of context (1280x720 after
# .NET shrinks it — see screen.py, where that number is measured).
#
...
# regardless. A cold capture (~2.6s measured, PowerShell plus first-use
# module prep) has headroom; this is the hard deadline for the case where
# the window server itself is wedged.
