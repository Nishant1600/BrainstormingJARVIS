...
def untrusted_tool_source(name: str) -> Optional[str]:
    ...
    if name in WEB_CONTENT_TOOLS:
        return "a web page"
    # opencode's native web tools arrive lowercase (`webfetch`,
    # `websearch`) rather than as `WebFetch`/`WebSearch`: same hole, same
    # gate. Only `bash` has been observed on the wire so far — these two
    # names are covered defensively, so that if/when opencode reports them,
    # turns through them taint from day one rather than acting untainted
    # until somebody notices. JARVIS's own reading tools mark explicitly
    # either way, so nothing hinges on this set being complete.
    if name.lower() in ("webfetch", "websearch"):
        return "a web page"
...
class BrainConfig:
    ...
    # Where the `opencode` binary lives for the opencode backend
    # (OpencodeBrain). Verbatim argv[0], never split. Plus a verbatim argv
    # prefix ahead of the `run` subcommand — the seam tests stand a script
    # in for the binary through, without a shell.
    opencode_path: Optional[str] = None
    opencode_argv_prefix: list[str] = field(default_factory=list)
...
#
# OpencodeBrain speaks `opencode run --format json` instead of holding one
# `claude -p` process open. The conversation persists server-side
# (opencode.db), so every turn is its own short-lived child against the same
# session id — measured live: a continued session answers from context, and
# the repeated system prompt is served from cache. A stuck turn therefore
# costs a kill and a retry, never the conversation.
#
# Everything stateful is the base class's, untouched: generations, the
# launch-prompt walls, handover wrapping, taint tracking, context budgets,
# restart budgets, and state callbacks. Only the transport differs, which is
# why this is a subclass and not a second brain. Three deliberate gaps,
# each documented where it bites:
#
# * No `--tools` flag exists on `opencode run`: the allowlist is enforced
#   through opencode.json (which names the jarvis MCP server) rather than
#   argv. ALLOWED_TOOLS stays as the declaration of intent.
# * No `--variant`/`--agent` flags are passed: variants are
#   provider-specific and a wrong one fails every turn, so effort tuning
#   waits for a provider mapping rather than guessing.
# * `live_tools`/`mcp_servers` stay empty: the CLI reports no inventory on
#   this channel. The status surface reads "nothing known", honestly, until
#   something queries `opencode mcp list` for it.

# An error event whose text matches this is a spent window, not a dead turn,
# matched case-insensitively. A clean turn afterwards clears it, exactly
# like the base class clears on a usable status event.
_RATE_LIMIT_RE = re.compile(r"rate.?limit|429|quota|too many requests",
                            re.IGNORECASE)
...
class OpencodeBrain(Brain):
    def __init__(self, config: BrainConfig):
        ...
        # Verbatim argv[0], never split — see RunExecutor for why.
        self._opencode = (config.opencode_path
                          or shutil.which("opencode") or "opencode")
        self._opencode_prefix = list(config.opencode_argv_prefix or [])
        # This generation's session. Survives stop(): the conversation lives
        # server-side, so a restart re-warms it instead of starting over.
        # Rotation is the only thing that retires one, and only once its
        # replacement has warmed up.
        self._sid: Optional[str] = None
        self._children: set[asyncio.subprocess.Process] = set()

    @staticmethod
    def child_env() -> dict[str, str]:
        return opencode_env.child_env()

    # -- model --
    def _resolve_model(self) -> Optional[str]:
        """provider/model for this brain, or None when unconfigured.

        opencode's `--model` takes provider/model and nothing else, so a
        bare family name ("sonnet", the claude default) is refused here
        with a message that names the fix, rather than handed to the CLI
        to fail opaquely on every turn.
        """
        raw = (os.getenv("JARVIS_BRAIN_MODEL", "") or self.config.model or "")
        model = raw.strip()
        return model if "/" in model else None

    # -- argv --
    def _argv(self, message: str) -> list[str]:
        """One turn's argv. List, never a shell string, binary never split."""
        model = self._resolve_model()
        if model is None:
            raise ValueError(
                "no model configured for the opencode brain: set "
                "JARVIS_BRAIN_MODEL to provider/model")
        cmd = [self._opencode] + self._opencode_prefix + [
            "run", "--format", "json",
        ]
        if self._sid is None:
            # The session list is how the user finds this conversation;
            # name it on creation only, never rename it mid-life.
            cmd += ["--title", "jarvis-brain"]
        else:
            cmd += ["--session", self._sid, "--continue"]
        # Always explicit --model (the base class's rule, kept); --auto is
        # this backend's `--dangerously-skip-permissions` — a voice turn
        # cannot answer an interactive prompt either way.
        #
        # --dir ALWAYS, and equal to the child's cwd: continuing a session
        # created under another directory hangs the CLI instead of answering
        # (measured, twice). Both point at the brain home, which is also
        # what gets the persona file read: CLAUDE.md there is loaded as
        # instructions, so the static half of the launch prompt needs no
        # transport of its own.
        return cmd + ["--model", model, "--auto",
                      "--dir", str(self.config.home), message]

    @property
    def running(self) -> bool:
        return (self._sid is not None and not self._stopping
                and not self._failed)

    # -- lifecycle --
    async def _spawn_locked(self, rotating: bool = False) -> bool:
        """Warm up (or re-warm) the session; True once it is serving.

        A fresh session (`_sid is None`) starts a generation: the launch
        prompt plus the warm-up ask go out as its first message. A re-warm
        sends the bare warm-up ask — the session already holds its context,
        and repeating the launch prompt would pollute it.
        """
        self.config.home.mkdir(parents=True, exist_ok=True)
        if self._resolve_model() is None:
            log.error("brain: no model configured for the opencode backend; "
                      "set JARVIS_BRAIN_MODEL to provider/model")
            self._failed = True
            await self._emit("failed", reason="no model configured")
            return False
        fresh = self._sid is None
        if fresh:
            self.generation += 1
            message = self.launch_prompt() + "\n\n" + WARMUP_TEXT
        else:
            message = WARMUP_TEXT
        run_warmup = self._turn_locked if rotating else self._turn
        warm = await run_warmup(message, "system", None,
                                timeout=self.config.warmup_timeout,
                                warmup=True)
        if warm.stop_reason != "result" or self._stopping:
            why = "stopping" if self._stopping else warm.stop_reason
            if warm.error:
                why = f"{why}: {warm.error}"
            log.error(f"brain: warm-up failed ({why})")
            fatal_reason = None if rotating else _classify_fatal_failure(
                warm.error)
            if fatal_reason and not self._stopping:
                self._failed = True
                self._failure_reason = fatal_reason
                await self._emit("failed", reason=why,
                                 failure_reason=fatal_reason)
            return False
        self._ready = True
        self.model_in_use = self._resolve_model()
        log.info(f"brain ready: gen={self.generation} model={self.model_in_use} "
                 f"session={self.session_id} ctx={self.context_tokens}")
        await self._emit("ready", generation=self.generation,
        ...
        if self._children:
            try:
                await asyncio.wait_for(
                    asyncio.gather(*(p.wait() for p in self._children),
                                   return_exceptions=True),
                    timeout=5.0)
            except asyncio.TimeoutError:
                log.warning("brain: turn children did not reap in time")
            self._children.clear()
        if self._inflight:
            self._inflight.finish("died")
        # The session id is deliberately KEPT: the conversation lives
        # server-side, and the next start() re-warms it.

    async def rotate(self, handover: Optional[str] = None) -> bool:
        """Replace the session with a fresh one carrying the handover.

        Same contract as the base: the turn lock serialises against flight,
        and a replacement that will not warm up leaves the current session
        serving — a mute JARVIS is worse than a full context window.
        """
        async with self._spawn_lock, self._turn_lock:
            if self._stopping or self._failed or not self.ready:
                log.info("rotation skipped: the brain is not serving")
                return False
            old_sid, old_gen, old_handover = (self._sid, self.generation,
                                              self._handover)
            old_gen_taint = self._generation_untrusted
            old_handover_taint = self._handover_untrusted
            self._handover = handover or self._handover
            if handover:
                self._handover_untrusted = self._generation_untrusted
            self._generation_untrusted = None
            self._sid = None       # detached, not deleted: it is the fallback
            self._rotating = True
            try:
                ok = await self._spawn_locked(rotating=True)
            finally:
            ...
            if not ok:
                ...
                self._handover_untrusted = old_handover_taint
                self._ready = True
                log.warning("rotation failed; keeping generation %d",
                            self.generation)
                return False
            self._rotation_pending = False
            self._turns_since_pending = 0
    ...
    async def _turn_locked(self, text, origin, on_delta, timeout,
                           warmup: bool = False, on_tool=None) -> TurnResult:
        """One message in, one completed turn out, through a per-turn child."""
        if self._failed or (not warmup and not self._ready):
            return TurnResult(origin, "", "not_running")
        if not warmup and self._rate_limited():
            return TurnResult(origin, "", "rate_limited",
                              rate_limit=self.rate_limit)
        try:
            argv = self._argv(text)
        except ValueError as e:
            return TurnResult(origin, "", "error", error=str(e))
        t = _Turn(origin, on_delta, None)
        t.on_tool = on_tool
        self._inflight = t
        try:
            try:
                proc = await asyncio.create_subprocess_exec(
                    *argv, cwd=str(self.config.home), env=self.child_env(),
                    stdin=asyncio.subprocess.DEVNULL,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                    limit=opencode_env.STREAM_LINE_LIMIT,
                )
            except OSError as e:
                log.error(f"brain: cannot start {self._opencode!r}: {e}")
                t.error = str(e)
                t.finish("died")
                if not warmup:
                    self._ready = False
                    self._schedule_restart("spawn failed")
                return self._release(t, warmup)
            self._children.add(proc)
            err_sink = bytearray()
            err_task = asyncio.create_task(
                self._drain_stderr(proc, err_sink))
            try:
                await asyncio.wait_for(self._consume_turn(proc, t), timeout)
            except asyncio.TimeoutError:
                t.finish("timeout")
                log.error(f"brain: turn stuck for {timeout}s; restarting")
                self._ready = False
                ...
                tail = await self._collect_stderr(err_task, err_sink)
                if t.stop_reason == "result" and proc.returncode not in (0, None):
                    t.error = tail or f"exit {proc.returncode}"
                    t.finish("error")
            return self._release(t, warmup)
        finally:
            if self._inflight is t:
    ...
    def _release(self, t: _Turn, warmup: bool) -> TurnResult:
        """The base _turn_locked epilogue, shared by construction: fold the
        turn's tokens into the window, and measure the resident floor off
        the warm-up turn — the one turn with no conversation in it."""
        if t.stop_reason == "result":
            self.context_tokens = t.context_tokens()
            if warmup:
                self.baseline_tokens = self.context_tokens
    ...
    async def _drain_stderr(self, proc: asyncio.subprocess.Process,
                            sink: bytearray) -> None:
        """Keep the stderr pipe empty so the child never blocks on it; the
        tail is what a failed turn reports."""
        try:
            while True:
    ...
    async def _collect_stderr(self, task, sink: bytearray) -> str:
        if task is not None and not task.done():
            try:
                ...
                pass
        return bytes(sink).decode(errors="replace")[-2000:]

    async def _consume_turn(self, proc: asyncio.subprocess.Process,
                            t: _Turn) -> None:
        """Drive one turn off the child's `--format json` stream.

        Usage is translated into the base class's key names as it arrives,
        so `context_tokens`, the resident floor, and rotation budgets work
        unchanged: opencode's `input`/`cache.read` become `input_tokens`/
        `cache_read_input_tokens`, and so on.
        """
        ...
        while True:
            ...
            kind = ev.get("type")
            if kind == "text":
                text = opencode_parser.text_of(ev)
                if text:
                    if t.first_delta is None:
                        t.first_delta = time.monotonic() - t.started
                    t.parts.append(text)
                ...
                name, _summary = opencode_parser.tool_of(ev)
                if name:
                    t.tools.append(name)
                    if t.on_tool:
                        try:
                ...
                step = opencode_parser.extract_assistant_usage(ev)
                usage["input_tokens"] += step["input_tokens"]
                usage["output_tokens"] += step["output_tokens"]
                usage["cache_read_input_tokens"] += step["cache_read_tokens"]
                usage["cache_creation_input_tokens"] += \
                    step["cache_creation_tokens"]
                t.usage = dict(usage)
            elif kind == "error":
                message = opencode_parser.extract_result_metrics(ev)[
                    "result_text"] or "opencode reported an error"
                t.error = message
                log.error(f"brain: turn failed: {message[:300]}")
                if _RATE_LIMIT_RE.search(message):
                    self.rate_limit = {
                        "status": "rate_limited",
                        "resetsAt": (time.time()
                                     + self.config.rate_limit_default_sec),
                        "message": message,
                    }
                    await self._emit("rate_limited",
                                     resets_at=self.rate_limit["resetsAt"])
                t.finish("error")
        if not t.done.is_set():
            t.usage = dict(usage)
            # A clean turn proves the window is usable again — the analogue
            # of the base class clearing on a usable status event.
            self.rate_limit = None
            t.finish("result")
