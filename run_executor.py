"""Spawns opencode runs and records everything they do.
...
# The model is always passed explicitly (`--model`): never rely on the
# CLI's own default for an unattended run nobody chose.
#
# `opencode run --format json` emits no terminal `result` event — the process
# exiting IS the result — so result text is accumulated off `text` events as
# the stream goes by, and capped at the width below.
_RESULT_TEXT_MAX = 20000
...
class RunExecutor:
    def __init__(self, store, opencode_path: str | None = None,
        ...
        self._store = store
        # The opencode binary is NEVER split: argv lists only, so a path
        # with spaces (Windows Program Files, npm shims) survives intact.
        # `opencode_argv_prefix` is prepended verbatim ahead of the `run`
        # subcommand — it exists so tests can stand a script in for the
        # binary (`[sys.executable, fake_script]`) without a shell.
        self._opencode = opencode_path or shutil.which("opencode") or "opencode"
        self._opencode_prefix = list(opencode_argv_prefix or [])
    ...
    def _resolve_model(self, model: str | None) -> str:
        """Explicit argument, else JARVIS_RUN_MODEL, else JARVIS_BRAIN_MODEL.

        Raises instead of falling back to the CLI's own default: an
        unattended run on an unknown model is a run nobody chose. The
        failure surfaces through start_existing's guarded handoff, so the
        row still reaches a terminal state with the reason in it.

        opencode's `--model` takes provider/model and nothing else, so a
        value without a slash (including a bare family name) is refused
        here with the fix named, rather than handed to the CLI to fail
        opaquely on every turn.
        """
        resolved = (model or os.getenv("JARVIS_RUN_MODEL")
                    or os.getenv("JARVIS_BRAIN_MODEL") or "").strip()
        if "/" not in resolved:
            raise ValueError(
                "no model configured for opencode runs: set JARVIS_RUN_MODEL "
                "or JARVIS_BRAIN_MODEL to provider/model")
        return resolved
    ...
    def _command(self, run_id: str, resume_sid: str | None,
                 model: str | None, prompt: str) -> list[str]:
        """`opencode run --format json ...` for one run. Argv list, never a
        shell string, and the binary is never split — see __init__.

        The prompt travels as the trailing positional argument: opencode
        takes its message on argv, not stdin. Resume continues a previous
        opencode SESSION (`ses_*`), looked up off the referenced run's own
        stored events — run ids and session ids are different namespaces.
        """
        cmd = [self._opencode] + self._opencode_prefix + [
            "run", "--format", "json",
        ]
        if resume_sid:
            cmd += ["--session", resume_sid, "--continue"]
        # Always explicit, never the CLI's own default — same reasoning as
        # _resolve_model above; empty here is refused before spawn.
        cmd += ["--model", self._resolve_model(model)]
        if _SKIP_PERMISSIONS:
            # Unattended-approval mode: without it a run parks on a
            # permission prompt it has no TTY to answer, and hangs until
            # the idle bound reaps it.
            cmd.append("--auto")
        cmd.append(prompt)
        return cmd

    def _lookup_opencode_session(self, resume_from: str | None) -> str | None:
        """The opencode `ses_*` id of a previous run, or None for a fresh start.

        Read off the referenced run's FIRST stored event: every `--format
        json` line carries the session id, so the stream JARVIS already
        persisted is the record. Anything missing or unparseable means a
        fresh start with a warning — a resume that names nothing it can
        continue must not fail the new run.
        """
        if not resume_from:
            return None
        try:
            events = self._store.get_events(resume_from, limit=1)
        except Exception as e:
            log.warning("run %s: could not read resume source: %s",
                        resume_from, e)
            return None
        if not events:
            return None
        ...
        if not isinstance(sid, str) or not sid:
            log.warning("run %s: resume source carries no session id; "
                        "starting fresh", resume_from)
            return None
        return sid
    ...
    async def _publish_run_updated(self, run_id: str) -> None:
        ...
        Only for changes that alter the row a client renders — the
        accumulated token usage, cost and step count from each
        `step_finish`, and result text off `text` events. NOT per streamed
        event: that would be one message per line of output, and
        `run_event` already covers the stream. Like every other
        transition, the store write has already happened; this is the
    ...
    async def _drive(self, run_id: str, prompt: str, project_path: str,
        ...
        try:
            ...
            # The prompt already travelled on argv; opencode takes its
            # message there, not on stdin. DEVNULL rather than a pipe: a
            # child that reads stdin would otherwise park on an open pipe
            # nobody writes to.
            resume_sid = await asyncio.to_thread(
                self._lookup_opencode_session, resume_from)
            argv = self._command(run_id, resume_sid, model, prompt)
            try:
                proc = await asyncio.create_subprocess_exec(
                    *argv,
                    ...
                    # Provider credentials MUST reach the child: opencode
                    # authenticates through the environment and auth.json.
                    # See opencode_env.py (the inverse of the old scrub,
                    # which is gone with the runtime it protected).
                    env=opencode_env.child_env(),
                    # asyncio's default 64 KiB StreamReader line limit is far
                    # smaller than a JSONL line can legitimately be
                    # (a big tool result, a long assistant message, a large
                    # diff). See opencode_env.STREAM_LINE_LIMIT.
                    limit=opencode_env.STREAM_LINE_LIMIT,
                )
            except (FileNotFoundError, NotADirectoryError, PermissionError) as e:
                await self._finish(run_id, self._store.RunStatus.FAILED,
                                   error=f"could not start opencode: {e}")
                return
            ...
            try:
                ...
                error = ("opencode produced no output for "
                         f"{self._idle_sec}s and was still running; killed")
                if stderr:
                    error = f"{error}\n{stderr}"
                await self._finish(run_id, self._store.RunStatus.TIMED_OUT,
                                   error=error)
            ...
            if run_id in self._cancelling:
                ...
                # refused to exit.
                error = ("opencode closed stdout but did not exit within "
                ...
                # Exit 0 is not a verdict. The CLI can report a failure as
                # an `error` event and STILL exit 0 — so a run that never
                # did anything would be recorded `succeeded`. A run whose
                # own stream says it errored is FAILED.
                error = (run["result_text"] or stderr
                         or "opencode reported an error mid-stream")
                await self._finish(run_id, self._store.RunStatus.FAILED,
    ...
    async def _consume(self, run_id: str, proc) -> None:
        ...
        # Tokens accumulated from the per-step usage on `step_finish`
        # events, so the dashboard can watch them climb — plus the summed
        # step costs (never estimated) and the step count. Result text
        # accumulates off `text` events, since no terminal event carries it.
        totals = {"input_tokens": 0, "output_tokens": 0,
                  "cache_read_tokens": 0, "cache_creation_tokens": 0}
        opencode_cost = 0.0
        opencode_steps = 0
        opencode_texts: list[str] = []
        # Sticky: once an `error` event has landed, `_drive` fails the run
        # even on exit 0, and a later clean step must not un-say it. The
        # stream is written by the CLI and not by the model, so this is
        # robustness rather than a live attack; it is still the wrong
        # direction to be wrong in. Once true, true.
        saw_error = False
        ...
        try:
            while True:
                ...
                # raised opencode_env.STREAM_LINE_LIMIT is not a guarantee,
                ...
                event = opencode_parser.parse_line(line)
                if event is None:
                    continue

                kind = opencode_parser.event_kind(event)
                # Capped, not stored verbatim: STREAM_LINE_LIMIT is 64 MiB
                # and jarvis.db is permanent. See opencode_parser.cap_payload —
                ...
                pending.append((seq, kind,
                                opencode_parser.cap_payload(line.strip(), event)))

                if kind == "step_finish":
                    # The per-step usage event: usage accumulates into the
                    # same columns the dashboard renders — the numbers climb
                    # live — plus the step's own cost and the step count.
                    # Dollars are never estimated, only summed from what
                    # the CLI reported.
                    usage = opencode_parser.extract_assistant_usage(event)
                    metrics = opencode_parser.extract_result_metrics(event)
                    opencode_steps += 1
                    opencode_cost += metrics["cost_usd"]
                    for key, value in usage.items():
                        totals[key] += value
                    await flush()
                    await asyncio.to_thread(
                        self._store.update_run, run_id,
                        cost_usd=opencode_cost, num_turns=opencode_steps,
                        **totals)
                    await self._publish_run_updated(run_id)
                elif kind == "text":
                    text = opencode_parser.text_of(event)
                    if text:
                        ...
                        await asyncio.to_thread(
                            self._store.update_run, run_id,
                            result_text=combined[-_RESULT_TEXT_MAX:])
                        await self._publish_run_updated(run_id)
                elif kind == "error":
                    # The CLI's own verdict, mid-stream: latch it, so exit 0
                    # with an error event still fails the run in _drive. A
                    # later clean step must not un-say an earlier failure.
                    metrics = opencode_parser.extract_result_metrics(event)
                    saw_error = True
                    await flush()
                    fields = {"is_error": 1}
                    if not opencode_texts and metrics["result_text"]:
                        fields["result_text"] = \
                            metrics["result_text"][:_RESULT_TEXT_MAX]
                    await asyncio.to_thread(
