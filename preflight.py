...
so `answer_dialog`'s keystroke fails, and no Fish Audio key at all. On the
opencode runtime (`JARVIS_RUNTIME=opencode`) the engine checks swap to
`opencode` on PATH with a new-enough `--version`, and a sign-in that lives
in auth.json or a provider key — a missing `claude` is then silence, not
an alarm, and a present `ANTHROPIC_*` is provider auth, not a leftover.
...
# The opencode series `run --format json` was verified against (1.18.32):
# the step_start/text/tool_use/step_finish/error vocabulary the run
# pipeline parses, `--session`/`--continue` resume, `--auto`, `--dir` and
# `--title` all date from this series as far as this project has measured.
# Older 1.x is refused rather than misparsed — a wrong event vocabulary
# would turn into wrong run verdicts, which is the dangerous direction.
MIN_OPENCODE_VERSION = (1, 18, 32)
MIN_OPENCODE_VERSION_STR = "1.18.32"
...
def _runtime_is_opencode() -> bool:
    """Whether spawned runs and the brain use opencode rather than claude.

    Read live (not cached) so tests and a reloaded server observe the same
    value the constructors do.
    """
    return os.getenv("JARVIS_RUNTIME", "claude").strip().lower() == "opencode"


async def _check_opencode_cli(timeout: float = DEFAULT_CHECK_TIMEOUT) -> Check:
    """`opencode` is on PATH and is at least MIN_OPENCODE_VERSION.

    Same shape as `_check_claude_cli` (missing → fail, unparseable →
    warn, too old → fail with an upgrade remedy) so the spoken summary
    and the dashboard render it the same way.
    """
    ...
    if not opencode:
        return Check(
            name="opencode_cli",
            status=STATUS_FAIL,
            message="`opencode` is not on PATH.",
            remedy=(
                "Install OpenCode and make sure it's on PATH, then verify "
                "with `opencode --version`."
            ),
        )

    rc, stdout, stderr = await _run_subprocess(opencode, "--version",
                                               timeout=timeout)
    if rc != 0:
        return Check(
            name="opencode_cli",
            status=STATUS_WARN,
            message=f"`opencode --version` failed: {(stderr or stdout).strip() or f'exit {rc}'}",
            remedy="Run `opencode --version` yourself to see what's wrong.",
        )
    ...
    if version is None:
        return Check(
            name="opencode_cli",
            status=STATUS_WARN,
            message=f"Could not parse a version from `opencode --version` output: {stdout.strip()!r}",
            remedy=f"Run `opencode --version` yourself and confirm it's {MIN_OPENCODE_VERSION_STR} or newer.",
        )
    ...
    if version < MIN_OPENCODE_VERSION:
        return Check(
            name="opencode_cli",
            status=STATUS_FAIL,
            message=f"opencode {version_str} is older than the required {MIN_OPENCODE_VERSION_STR}.",
            remedy="Update OpenCode: opencode upgrade",
        )

    return Check(name="opencode_cli", status=STATUS_OK,
                 message=f"opencode {version_str} on PATH.")


# Provider credentials opencode honours out of the environment, as a
# fallback when auth.json holds nothing. Deliberately short: these are the
# widely-documented `*_API_KEY` names, and every entry here is checked for
...
def _opencode_auth_path() -> Path:
    """Where opencode keeps its login: `~/.local/share/opencode/auth.json`
    on POSIX, `%USERPROFILE%\\.local\\share\\opencode\\auth.json` on
    Windows — both spellings are `Path.home() / ".local" / ...`, so one
    expression covers both. Factored out (like `_settings_path`) so tests
    can point it at a tmp file instead of the developer's real login."""
    return Path.home() / ".local" / "share" / "opencode" / "auth.json"


def _check_opencode_auth_sync() -> Check:
    """opencode can authenticate a turn: auth.json holds a login, or a
    provider key is in the environment.

    Synchronous: two cheap local reads, no subprocess. Presence only —
    the file is parsed as JSON and counted as non-empty, and env keys are
    tested for non-blank values; no credential value ever reaches a
    ...
    try:
        ...
        return Check(
            ...
            remedy="Run `opencode auth login` yourself to confirm you're signed in.",
        )
    if raw.strip():
        try:
            data = json.loads(raw)
            ...
            return Check(
                name="opencode_auth",
                status=STATUS_WARN,
                message=f"Could not parse {path}; cannot verify the sign-in.",
                remedy="Run `opencode auth login` yourself to confirm you're signed in.",
            )
        if isinstance(data, dict) and data:
            return Check(name="opencode_auth", status=STATUS_OK,
                         message=f"Signed in ({path}).")

    present = sorted(k for k in _OPENCODE_PROVIDER_ENV_KEYS
                     if os.getenv(k, "").strip())
    if present:
        return Check(
            name="opencode_auth", status=STATUS_OK,
            message=("Signed in with a provider key from the environment "
                     f"({path} holds no login)."))

    return Check(
        name="opencode_auth",
        status=STATUS_FAIL,
        message="OpenCode is not signed in: no login in auth.json and no provider key in the environment.",
        remedy="Run `opencode auth login` — the voice brain runs on that login, not on anything in .env.",
    )
...
async def _check_accessibility(timeout: float = DEFAULT_CHECK_TIMEOUT) -> Check:
    ...
    On Windows the same question is asked of the console backend: can this
    process open (and therefore answer in) a console at its own integrity
    level? No permission dialog exists to trigger here either — the probe
    below opens nothing and presses nothing.
    """
    if sys.platform == "win32":
        return await _check_windows_console_automation()
...
async def _check_windows_console_automation(
        timeout: float = DEFAULT_CHECK_TIMEOUT) -> Check:
    """Whether this process could answer a dialog in a console window.

    The Windows half of `_check_accessibility`: `dialog_win` presses via
    SendInput into a console attached with AttachConsole, which fails
    across integrity levels (an unelevated server cannot reach an elevated
    console) and where there is no console at all. The probe opens and
    presses nothing — it asks the backend whether OUR OWN pid is live and
    reachable, which exercises the same OpenProcess path a real answer
    would take, minus the keystroke.

    Takes `timeout` for signature parity with the other async checks; the
    probe itself is two local syscalls.
    """
    _ = timeout
    try:
        import dialog
        ...
        return Check(
            name="accessibility",
            status=STATUS_WARN,
            message=f"Could not determine console-automation status: {e}")
    try:
        state = await asyncio.to_thread(dialog._backend.pid_state,
                                        os.getpid())
    except Exception as e:
        return Check(
            name="accessibility",
    ...
    if state == "live":
        return Check(name="accessibility", status=STATUS_OK,
                     message="Console automation is available: this process "
                             "can open its own console.")
    if state == "denied":
        return Check(
            name="accessibility",
            status=STATUS_FAIL,
            message="This process cannot open console windows at a higher "
                    "integrity level; answer_dialog's keypress will fail there.",
            remedy=("Run JARVIS and the session consoles at the same "
                    "integrity level — both unelevated, or both elevated — "
                    "rather than mixing an elevated terminal with a plain one."),
        )
    return Check(
        name="accessibility",
        status=STATUS_WARN,
        message="Could not determine console-automation status from here.")
...
def _check_screen_recording_sync() -> Check:
    ...
    On Windows there is no per-app grant to ask about, so a healthy machine
    is silent here: the capability is reported available, and the
    blank-frame check at the moment of asking carries the load. An explicit
    False (a test double today, a restricted context tomorrow) still fails
    loudly rather than being rounded up to OK.
    """
    try:
    ...
    if sys.platform == "win32":
        if granted is False:
            return Check(
                ...
                status=STATUS_FAIL,
                message="Screen capture is not available to JARVIS on this "
                        "machine; look_at_screen will refuse.",
                remedy=("Screen Recording is attributed to the app that "
                        "launched JARVIS, not to python. Restart that app if "
                        "its capabilities changed recently; otherwise report "
                        "the exact error rather than changing settings."),
            )
        return Check(name="screen_recording", status=STATUS_OK,
                     message="Screen capture needs no permission grant on Windows.")
...
# The opencode runtime swaps the engine checks: a claude binary that is
# missing is not a problem when nothing will spawn it (and warning about
# it on every boot would train the user to ignore preflight), while an
# ANTHROPIC_* variable in .env is EXPECTED provider auth rather than a
# leftover, and `crossSessionInbound` in ~/.claude/settings.json is read
# by nothing on this backend. Shared checks (Fish key, screen state) and
# the accessibility probe run on both.
_OPENCODE_ASYNC_CHECKS = (_check_opencode_cli, _check_accessibility)
_OPENCODE_SYNC_CHECKS = (_check_fish_api_key_sync, _check_opencode_auth_sync,
...
async def run_checks(*, timeout: float = DEFAULT_CHECK_TIMEOUT) -> list[Check]:
    ...
    The engine checks follow JARVIS_RUNTIME: an opencode boot runs the
    opencode CLI/auth probes instead of the claude ones, so a healthy
    machine reports nothing on either backend.
    ...
    """
    if _runtime_is_opencode():
        async_fns, sync_fns = _OPENCODE_ASYNC_CHECKS, _OPENCODE_SYNC_CHECKS
    else:
        async_fns, sync_fns = _ASYNC_CHECKS, _SYNC_CHECKS
    tasks = [_run_one(fn, is_async=True, timeout=timeout) for fn in async_fns]
    tasks += [_run_one(fn, is_async=False, timeout=timeout) for fn in sync_fns]
...
def _phrase_for(check: Check) -> str:
    ...
    if name == "opencode_cli":
        if "not on PATH" in msg:
            return "OpenCode isn't installed"
        if "older than" in msg:
            return "OpenCode needs updating"
        return "OpenCode's version couldn't be checked"
    if name == "opencode_auth":
        if "not signed in" in msg:
            return "OpenCode isn't signed in"
        return "OpenCode's sign-in couldn't be checked"
    if name == "accessibility":
        if "not granted Accessibility" in msg:
            return "I don't have Accessibility permission"
        if "integrity level" in msg:
            return "I can't reach consoles running as administrator"
