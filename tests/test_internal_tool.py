...
@pytest.mark.skipif(sys.platform == "win32",
                        reason="Windows readability is governed by ACLs, not "
                               "mode bits: st_mode cannot show 0o600")
...
@pytest.mark.skipif(sys.platform == "win32",
                        reason="Windows readability is governed by ACLs, not "
                               "mode bits: chmod cannot produce 0o600")
...
def test_a_symlink_in_the_token_path_is_refused_not_followed(client, tmp_path):
    ...
    try:
        ...
        _pytest.skip("this machine may not create symlinks "
                     "(Windows needs privilege or Developer Mode)")
...
def test_a_pre_existing_token_is_adopted_across_calls(client):
    """The restart path: a token written by an earlier boot is returned
    unchanged by the next one, on every platform (this is the adopt branch
    `ensure_tool_token` takes once the file exists)."""
    c, server = client
    first = server.data_paths.ensure_tool_token()
    assert server.data_paths.tool_token_path().read_text() == first
    assert server.data_paths.ensure_tool_token() == first


def test_a_link_shaped_path_is_refused_even_where_no_follow_is_missing(
        client, monkeypatch):
    """Wires the Windows symlink refusal without needing the privilege to
    make a real link: where `O_NOFOLLOW` does not exist, a path that
    *looks* like a link must raise before any open can follow it."""
    import pytest as _pytest
    c, server = client
    server.data_paths.ensure_tool_token()  # the real file exists now
