...
EXEMPT = {
    ...
    "opencode_watch._recap": (
        "this module is the SOURCE of the foreign values on the opencode "
        "backend, the way session_watch is on the claude one: it reads "
        "message and part rows and returns a Recap. It speaks to nobody — "
        "its only caller is `_one_session`, which puts the recap into a "
        "SessionState whose every name is then composed by the shared "
        "machinery the wall test drives"),
    "opencode_watch.build_snapshot": (
        "it SORTS conversations by (project, voice name) and returns "
        "Snapshot objects — the f-string in it is a log line naming the "
        "database file. What is said about the sessions comes from "
        "server's driven clauses, same as for `session_watch.resolve` "
        "above"),
    ...
    # --- the opencode wire shapes, not the records they are named like -----
    #
    # `opencode_parser` reads `state`, `error` and `message` off `--format
    # json` EVENTS — a tool part's status, a failed run's message, a text
    # part — and those names collide with a run row's columns and a
    # session's fields. The walk cannot tell them apart, so this says it:
    # no session and no run row is ever read here, only the CLI's own
    # stream, whose every string is somebody else's words by construction.
    "opencode_parser.tool_of": (
        "it returns a (tool name, input summary) pair to its one caller, "
        "`summarize_event`, below. Both halves are tool-output text, and "
        "the pair travels in run_event payloads, which the dashboard "
        "renders through textContent and which are never headered"),
    "opencode_parser.summarize_event": (
        "the opencode twin of `stream_parser.summarize_assistant`, which "
        "the walk passes over: it reads `text`/`tool`/`error` off the wire "
        "event and returns 160 clipped, single-line characters for the "
        "live feed. Newlines never survive it — the text branch splits "
        "and rejoins, the tool branch is built from an already-joined "
        "summary — so there is no line in its output to break a header "
        "with, and like its twin it is never given one"),
    "brain._consume_turn": (
        "it subscripts `result_text` off the metrics dict "
        "`opencode_parser.extract_result_metrics` built from the wire "
        "event two lines above — never off a run row. The value becomes "
        "the turn's error text, which travels the failure path (log line "
        "and spoken apology), never an unwrapped header: the only "
        "system-prose slot model output can reach is the handover block, "
        "which is always wrapped as untrusted"),
}
