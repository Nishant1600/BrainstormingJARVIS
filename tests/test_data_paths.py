...
def test_an_unedited_older_persona_is_brought_up_to_date(monkeypatch, tmp_path):
    ...
    # Exact bytes: the sync decision hashes file bytes, and text-mode writes
    # translate newlines on Windows.
    dp.persona_path().write_bytes(old.encode("utf-8"))
...
def test_first_run_updates_a_persona_it_can_prove_is_a_shipped_template(
    ...
    # Exact bytes — see test_an_unedited_older_persona_is_brought_up_to_date.
    dp.persona_path().write_bytes(old.encode("utf-8"))
