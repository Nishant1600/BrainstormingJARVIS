...
NO TEST IN THIS FILE MAY INVOKE THE REAL `claude` OR `opencode` BINARIES,
THE REAL `osascript`, OR READ THE DEVELOPER'S REAL ~/.claude/settings.json
OR AUTH FILE.
...
settings-file check goes through `preflight._settings_path` and the auth
check through `preflight._opencode_auth_path`, which tests point at tmp_path
...
# --- Windows console automation (the accessibility analogue) ------------------
...
class _WinBackend:
    ...
    def press_in_console(self, pid, normalized):  # pragma: no cover
        raise AssertionError("the probe opens nothing and presses nothing")


@pytest.mark.asyncio
async def test_windows_console_automation_ok_when_reachable(monkeypatch):
    """The probe asks about OUR OWN pid through the same backend a real
    answer would use — opens nothing, presses nothing."""
    import dialog
    monkeypatch.setattr(preflight.sys, "platform", "win32")
    fake = _WinBackend("live")
    monkeypatch.setattr(dialog, "_backend", fake)
    check = await preflight._check_accessibility()
    assert check.status == STATUS_OK
    assert check.remedy is None
    assert fake.seen and all(isinstance(p, int) for p in fake.seen)


@pytest.mark.asyncio
async def test_windows_console_automation_fails_across_integrity(monkeypatch):
    import dialog
    monkeypatch.setattr(preflight.sys, "platform", "win32")
    monkeypatch.setattr(dialog, "_backend", _WinBackend("denied"))
    check = await preflight._check_accessibility()
    assert check.status == STATUS_FAIL
    assert check.remedy
    assert "integrity level" in check.message


@pytest.mark.asyncio
async def test_windows_console_automation_probe_failure_is_warn(monkeypatch):
    import dialog

    class _Boom(_WinBackend):
        def pid_state(self, pid):
            raise OSError("conhost is gone")

    monkeypatch.setattr(preflight.sys, "platform", "win32")
    monkeypatch.setattr(dialog, "_backend", _Boom("live"))
    check = await preflight._check_accessibility()
    assert check.status == STATUS_WARN


def test_windows_screen_capture_needs_no_grant(monkeypatch):
    """Unknown probe on Windows means available (there is no grant), so a
    healthy boot stays silent — the opposite of the macOS rule, where
    unknown means warn."""
    monkeypatch.setattr(preflight.sys, "platform", "win32")
...
def test_spoken_summary_names_the_integrity_gap():
    checks = [Check(name="accessibility", status=STATUS_FAIL,
...
def test_screen_recording_undeterminable_is_warn_not_fail(monkeypatch):
    """None means the probe could not run -- on macOS, or a macOS that moved
    the symbol. Reporting that as a missing permission would send the user to
    a settings pane over nothing. (Scoped to darwin: on Windows an unknown
    probe means available, covered below.)"""
    monkeypatch.setattr(preflight.sys, "platform", "darwin")
...
# --- opencode_cli: PATH + version (same shape as claude_cli) ------------------

@pytest.mark.asyncio
async def test_opencode_missing_from_path(monkeypatch):
    monkeypatch.setattr(preflight.shutil, "which", lambda name: None)
    check = await preflight._check_opencode_cli()
    assert check.status == STATUS_FAIL
    assert "PATH" in check.message
    assert check.remedy
...
@pytest.mark.asyncio
async def test_opencode_present_but_too_old(monkeypatch):
    monkeypatch.setattr(preflight.shutil, "which", lambda name: "/usr/local/bin/opencode")
    monkeypatch.setattr(
        preflight, "_run_subprocess",
        _fake_run_subprocess({("/usr/local/bin/opencode", "--version"): (0, "1.17.0\n", "")}),
    )
    check = await preflight._check_opencode_cli()
    assert check.status == STATUS_FAIL
    assert "1.17.0" in check.message
    assert check.remedy


@pytest.mark.asyncio
async def test_opencode_present_and_new_enough(monkeypatch):
    monkeypatch.setattr(preflight.shutil, "which", lambda name: "/usr/local/bin/opencode")
    monkeypatch.setattr(
        preflight, "_run_subprocess",
        _fake_run_subprocess({("/usr/local/bin/opencode", "--version"): (0, "1.19.0\n", "")}),
    )
    check = await preflight._check_opencode_cli()
    assert check.status == STATUS_OK
    assert check.remedy is None
...
@pytest.mark.asyncio
async def test_opencode_exactly_at_minimum_version_is_ok(monkeypatch):
    monkeypatch.setattr(preflight.shutil, "which", lambda name: "/usr/local/bin/opencode")
    monkeypatch.setattr(
        preflight, "_run_subprocess",
        _fake_run_subprocess({("/usr/local/bin/opencode", "--version"): (0, "1.18.32\n", "")}),
    )
    check = await preflight._check_opencode_cli()
    assert check.status == STATUS_OK


def test_opencode_version_comparison_is_numeric_not_lexicographic():
    """The same trap as the claude check: "1.18.9" reads greater than
    "1.18.32" as a string, while 9 < 32 as a version component."""
    assert preflight._parse_version("1.18.9") < preflight.MIN_OPENCODE_VERSION
    assert preflight._parse_version("1.18.32") == preflight.MIN_OPENCODE_VERSION
    assert "1.18.9" > "1.18.32"  # the naive (wrong) string comparison, for contrast


@pytest.mark.asyncio
async def test_opencode_version_unparseable_output_is_warn(monkeypatch):
    monkeypatch.setattr(preflight.shutil, "which", lambda name: "/usr/local/bin/opencode")
    monkeypatch.setattr(
        preflight, "_run_subprocess",
        _fake_run_subprocess({("/usr/local/bin/opencode", "--version"): (0, "garbage\n", "")}),
    )
    check = await preflight._check_opencode_cli()
    assert check.status == STATUS_WARN


# --- opencode_auth: auth.json or a provider key, values never surface --------

def _auth_file(monkeypatch, tmp_path, content=None):
    """Point the auth check at a tmp file: a dict for `content` is written
    as JSON, any other value is written verbatim, None means no file."""
    ...
    monkeypatch.setattr(preflight, "_opencode_auth_path", lambda: p)
    for var in preflight._OPENCODE_PROVIDER_ENV_KEYS:
        monkeypatch.delenv(var, raising=False)
    return p


def test_opencode_auth_ok_with_a_login_file(monkeypatch, tmp_path):
    _auth_file(monkeypatch, tmp_path, {"opencode": {"token": "deadbeef"}})
    check = preflight._check_opencode_auth_sync()
    assert check.status == STATUS_OK
    assert check.remedy is None
    assert "deadbeef" not in check.message


def test_opencode_auth_values_never_reach_the_message(monkeypatch, tmp_path):
    """The check counts a login; it must not repeat one. A distinct marker
    stands in for a secret, and the message is searched for it."""
    marker = "SECRET-MARKER-9f2c1b"
    p = _auth_file(monkeypatch, tmp_path, {"provider": {"apiKey": marker}})
    check = preflight._check_opencode_auth_sync()
    assert check.status == STATUS_OK
    assert marker not in check.message
    # Read-only, like every other check in this module.
    assert marker in p.read_text(encoding="utf-8")


def test_opencode_auth_ok_with_a_provider_key_and_no_file(monkeypatch, tmp_path):
    _auth_file(monkeypatch, tmp_path)
...
def test_opencode_auth_fails_with_neither(monkeypatch, tmp_path):
    _auth_file(monkeypatch, tmp_path)
    check = preflight._check_opencode_auth_sync()
    assert check.status == STATUS_FAIL
    assert "not signed in" in check.message
    assert check.remedy and "opencode auth login" in check.remedy


def test_opencode_auth_unparseable_file_is_warn_not_fail(monkeypatch, tmp_path):
    """A corrupt file is "cannot verify", not "not signed in": claiming the
    latter would send a signed-in user to re-authenticate."""
    _auth_file(monkeypatch, tmp_path, "{not json")
    check = preflight._check_opencode_auth_sync()
    assert check.status == STATUS_WARN
    assert check.remedy


def test_opencode_auth_unreadable_path_is_warn(monkeypatch, tmp_path):
    p = _auth_file(monkeypatch, tmp_path)
    monkeypatch.setattr(preflight, "_opencode_auth_path", lambda: p / "a-dir")
    (p / "a-dir").mkdir(parents=True)
    check = preflight._check_opencode_auth_sync()
    assert check.status == STATUS_WARN


# --- run_checks follows JARVIS_RUNTIME -----------------------------------------

def test_run_checks_select_opencode_engine_when_selected(monkeypatch):
    monkeypatch.setenv("JARVIS_RUNTIME", "opencode")
    assert preflight._runtime_is_opencode() is True
    names = [fn.__name__ for fn in preflight._OPENCODE_ASYNC_CHECKS]
    assert names == ["_check_opencode_cli", "_check_accessibility"]
    names = [fn.__name__ for fn in preflight._OPENCODE_SYNC_CHECKS]
    assert names == ["_check_fish_api_key_sync", "_check_opencode_auth_sync",
                     "_check_screen_recording_sync"]
    assert not any("claude" in n or "anthropic" in n or "cross_session" in n
                   for n in names)


def test_run_checks_default_to_the_claude_engine(monkeypatch):
    monkeypatch.delenv("JARVIS_RUNTIME", raising=False)
    assert preflight._runtime_is_opencode() is False


@pytest.mark.asyncio
async def test_run_checks_on_opencode_never_mentions_claude(monkeypatch, tmp_path):
    """End to end through run_checks with everything faked: a healthy
    opencode machine is silent about claude, and vice versa is covered by
    test_run_checks_runs_all_registered_checks above."""
    monkeypatch.setenv("JARVIS_RUNTIME", "opencode")
    monkeypatch.setattr(preflight.shutil, "which",
                        lambda name: "/bin/" + name)
    monkeypatch.setattr(
        preflight, "_run_subprocess",
        _fake_run_subprocess({
            ("/bin/opencode", "--version"): (0, "1.18.32\n", ""),
            ("osascript", "-e"): (0, "window1\n", ""),
        }),
    )
    _auth_file(monkeypatch, tmp_path, {"opencode": {"ok": True}})
    ...
    results = await preflight.run_checks(timeout=1.0)
    names = {c.name for c in results}
    assert names == {"opencode_cli", "accessibility", "fish_api_key",
                     "opencode_auth", "screen_recording"}
    assert preflight.spoken_summary(results) == ""


def test_spoken_summary_names_opencode_problems_in_plain_words():
    checks = [
        Check(name="opencode_cli", status=STATUS_FAIL,
              message="`opencode` is not on PATH."),
        Check(name="opencode_auth", status=STATUS_FAIL,
              message="OpenCode is not signed in: no login anywhere."),
    ]
    summary = preflight.spoken_summary(checks)
    assert "Two things need attention, sir" in summary
    assert "OpenCode isn't installed" in summary
    assert "OpenCode isn't signed in" in summary
