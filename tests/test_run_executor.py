...
from tests.conftest import quoted_cmd
...
def _fake_claude(tmp_path: Path, fixture: Path, exit_code: int = 0,
    ...
    return quoted_cmd(sys.executable, script)
...
async def test_store_failure_still_reaches_terminal_state(env):
    ...
    assert ex._procs == {}
    from tests.conftest import pid_alive
    assert not pid_alive(pid), f"child {pid} still alive"
...
async def test_timeout_keeps_the_collected_stderr(env, tmp_path):
    ...
    ex = mod.RunExecutor(store, claude_path=quoted_cmd(sys.executable, script),
                         grace_sec=1.0)
    run_id = await ex.spawn("hang", "proj", str(tmp), "api", timeout_sec=1)
    run = await ex.wait_for(run_id, timeout=20)
...
async def test_a_run_still_gets_the_ordinary_environment(env, monkeypatch):
    ...
    # The home variable is HOME on POSIX and USERPROFILE on Windows; either
    # way the child must inherit the user's own, not lose it to the scrub.
    home_key = "USERPROFILE" if sys.platform == "win32" else "HOME"
    assert child.get(home_key) == os.environ[home_key]
...
def _exits_leaving_a_grandchild(tmp_path: Path, name: str,
    ...
    return _script(
        ...
        "sys.stdout.write(%r + '\\n')\n"
        "sys.stdout.flush()\n"
        # The interpreter itself, not `sleep`: there is no sleep binary on
        # Windows, and the point is a descendant holding the pipe, not which
        # program it is.
        "subprocess.Popen([sys.executable, '-c',"
        " 'import time; time.sleep(10)']%s)\n"
