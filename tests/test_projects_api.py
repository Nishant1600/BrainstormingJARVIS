...
def test_open_in_browser_calls_actions_with_a_file_uri(wired, monkeypatch,
                                                         tmp_path):
    """`as_uri` needs a genuinely absolute path: the POSIX-style fake used
    elsewhere in this file is relative on Windows (no drive letter) and
    raises there instead of producing a file:// URI. The dashboard always
    sends real absolute project paths, so the test does too."""
    server, _store = wired
    fake = _Actions()
    ...
    with TestClient(server.app, headers=BROWSER) as c:
        monkeypatch.setattr(server, "actions", fake)
        server.session_watcher = _Watcher([_session("s1", "chitauri",
                                                    str(project))])
        r = c.post("/api/projects/open",
                   json={"name": "chitauri", "path": str(project),
                         "target": "browser"})
    assert r.status_code == 200
    assert fake.browser == [Path(str(project)).as_uri()]
