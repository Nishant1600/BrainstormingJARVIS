...
class SessionWatcher:
    ...
    def build_snapshot(self, now: float) -> Snapshot:
        """This poll's snapshot. A method (not the module function) so the
        opencode watcher can source its sessions elsewhere while reusing
        every line below: gone-cache retention, naming, primary ranking,
        and transition announcements."""
        return build_snapshot(roots=self.roots, now=now)
