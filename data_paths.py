...
def _write_atomically(path: Path, text: str) -> bool:
    ...
    Written as UTF-8 BYTES, not text: text mode translates `\n` to the OS
    line separator on write, which on Windows turns every LF into CRLF. The
    sync decision above hashes exact bytes (`live_hash == recorded`), so a
    translated write would never match the hash of what was meant — every
    file JARVIS writes would read back as "edited" on the next boot.
    """
    try:
...
def _sync_template(template: Path, target: Path, seed: Path,
    ...
    * An EDITED file must never be silently overwritten. We know an edit by
    ...
    try:
        # Explicit UTF-8: the locale default decodes these template bytes
        # into different characters on Windows, and the hash below (plus
        # every byte written from this string) must agree across platforms.
...
def ensure_tool_token() -> str:
    ...
    Windows: `O_NOFOLLOW`, `os.getuid` and `os.fchmod` do not exist there, so
    the adopt path degrades in three documented ways. (1) A symlink at the
    path is refused up front with `os.path.islink` — same refusal as
    `O_NOFOLLOW` gives on POSIX, minus its atomicity: a link swapped in
    between the check and the open would still be followed, a race a local
    attacker would have to win against a server restart. (2) The ownership
    check is skipped: the token lives under the user's own profile/data dir,
    whose ACLs are the boundary, and there is no uid to compare against.
    (3) The 0600 tightening goes through `os.chmod`, which on Windows only
    toggles the read-only flag — the file is created non-read-only and stays
    that way, with ACL inheritance from the user's profile doing the real
    access control.
    """
    import secrets
    ...
    if not hasattr(os, "O_NOFOLLOW") and os.path.islink(str(path)):
        # No atomic no-follow open on this platform: refuse the link before
        # the open below can follow it (see the docstring for the residual
        # race). On POSIX this branch never runs — O_NOFOLLOW raises ELOOP
        # instead, atomically.
        raise OSError(f"{path} is a symlink; refusing to adopt it")
    ...
    try:
        ...
        if hasattr(os, "fchmod"):
            ...
            # Windows has no fd chmod: path-based, best-effort, and only
            # reached for a non-link (checked above), so this cannot be
            # redirected at a file the user did not mean.
            try:
